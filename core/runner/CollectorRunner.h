...
public:
  ...
  /// </summary>
  /// <param name="sourceNames">List of collector names to run ("all" = run
  /// all)</param> <param name="outEntries">Output: collected entries</param>
  /// <param name="outErrors">Output: collection errors</param>
  /// <returns>Number of collectors that successfully executed</returns>
  int Run(const std::vector<std::string> &sourceNames,
...
private:
  ...
  bool ShouldRun(const std::string &collectorName,
