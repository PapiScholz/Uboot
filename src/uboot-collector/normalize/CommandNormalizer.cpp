...
std::wstring CommandNormalizer::TrimWhitespace(const std::wstring& input) {
    ...
    while (start != input.end() && std::iswspace(*start)) {
        ++start;
    }
...
std::wstring CommandNormalizer::ExpandEnvironmentVariables(const std::wstring& input) {
    // First, try simple expansion up to a reasonable size
    wchar_t expanded[32768];
    DWORD result = ExpandEnvironmentStringsW(input.c_str(), expanded, _countof(expanded));
...
void CommandNormalizer::ParseCommandLine(
...
std::wstring CommandNormalizer::CanonicalizePath(
    ...
    if (!PathIsAbsoluteW(path.c_str()) && !workingDir.empty()) {
    ...
    HRESULT hr = PathCchCanonicalizeEx(canonical, _countof(canonical), resolved.c_str(), 0);
    if (SUCCEEDED(hr)) {
        resolved = canonical;
    } else {
        // Fallback: use GetFullPathNameW
        wchar_t fullPath[MAX_PATH + 1];
        DWORD len = GetFullPathNameW(resolved.c_str(), _countof(fullPath), fullPath, nullptr);
...
bool CommandNormalizer::TryPeelSingleWrapper(
    ...
    if (exeName == L"cmd.exe" || exeName == L"cmd") {
        if (arguments.find(L"/c") == 0 || arguments.find(L"/C") == 0) {
            ...
            while (cmdStart < arguments.length() && std::iswspace(arguments[cmdStart])) {
                cmdStart++;
            }
    ...
    if (exeName == L"powershell.exe" || exeName == L"powershell") {
        ...
        if (cmdPos != std::wstring::npos) {
            ...
            while (cmdStart < args.length() && std::iswspace(args[cmdStart])) {
                cmdStart++;
            }
        ...
        if (encPos != std::wstring::npos) {
            ...
            if (cmdStart < args.length()) {
                ...
                while (cmdEnd < args.length() && !std::iswspace(args[cmdEnd])) {
                    cmdEnd++;
                }
                outArguments = args.substr(cmdStart, cmdEnd - cmdStart);
                outExecutable = L"powershell.exe";
                outNote = L"Found: powershell.exe -EncodedCommand (base64 extracted, not decoded/executed)";
    ...
    if (exeName == L"regsvr32.exe" || exeName == L"regsvr32") {
        ...
        while (tokenStart < trimmedArgs.length()) {
            if (trimmedArgs[tokenStart] == L'/') {
                ...
                tokenStart = tokenEnd;
                while (tokenStart < trimmedArgs.length() && std::iswspace(trimmedArgs[tokenStart])) {
                    tokenStart++;
                }
            } else {
                break;
...
void CommandNormalizer::PeelWrappers(
    ...
    for (int iteration = 0; iteration < MAX_ITERATIONS; ++iteration) {
        ...
        if (TryPeelSingleWrapper(executable, arguments, newExecutable, newArguments, note)) {
            executable = newExecutable;
            arguments = newArguments;
            notes.push_back(note);
        } else {
            // No wrapper found or cannot peel further (e.g., powershell -EncodedCommand)
            if (!note.empty()) {
                notes.push_back(note);
            }
            break;
...
NormalizationResult CommandNormalizer::Normalize(
    ...
    if (!IsExecutableFile(executable)) {
        result.isComplete = false;
        result.resolveNotes += "Warning: Resolved path does not exist or is not a file. ";
    }
